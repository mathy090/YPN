...
import { kvGet, kvGetFresh, kvSet, kvSetTTL } from "../utils/db";

// ── Config ─────────────────────────────────────────────────────────────────────
const API_URL = process.env.EXPO_PUBLIC_API_URL ?? "";
const PREFETCH_COUNT = 3;
const MANIFEST_TTL_SEC = 60 * 60; // 1 hour
const { width: W, height: H } = Dimensions.get("window");
const STATUS_H =
  Platform.OS === "android" ? (RNStatusBar.currentHeight ?? 24) : 0;

// ── Cache keys ────────────────────────────────────────────────────────────────
const MK_DATA = "drive_manifest_v4";
const localPathKey = (id: string) => `drive_lp_${id}`;
...
// ── Local file cache ──────────────────────────────────────────────────────────
const LOCAL_DIR = FileSystem.cacheDirectory + "ypn_drive_v4/";
...
async function readLP(id: string): Promise<string | null> {
  return kvGet<string>(localPathKey(id));
}

async function saveLP(id: string, path: string): Promise<void> {
  await kvSet(localPathKey(id), path);
}
  ...
  ({
    ...
    useEffect(() => {
      isFileCached(item.fileId).then((ok) => {
        if (ok) readLP(item.fileId).then(setLocalPath);
      });
    }, [item.fileId]);
...
export default function ForYouScreen() {
  ...
  async function boot() {
    // Try SQLite TTL cache
    const cached = await kvGetFresh<DriveVideo[]>(MK_DATA);
  ...
  async function refreshManifest(manual: boolean) {
    ...
    try {
      ...
      await kvSetTTL(MK_DATA, data, MANIFEST_TTL_SEC);
      setVideos(data);
      prefetch(data);
    } catch {
      const stale = await kvGetFresh<DriveVideo[]>(MK_DATA);
